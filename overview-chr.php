...
<?php /*
    require("config.php");
    $noOfGuest = "SELECT guest FROM overview WHERE userID=".$userID;
    $resultGuest = $con->query($noOfGuest);

    if($resultGuest !== false && $resultGuest->num_rows > 0){
        $rowOfGuest = $resultGuest->fetch_assoc();
        $guest = $rowOfGuest['guest'];
    }
    $con->close(); */
?>
                    ...
                    ?>
                      <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="POST">
                            <div class="overview_content">
                                <input type="text" name="bride" placeholder="Bride's name">
                            ...
                            <input type="submit" name="confirm1">
                        </form>
                    <?php
                ...
                <div class="overview_content">
                    ...
                    <?php
                        ...
                        for($i; $i < 12; ++$i){
                            ...
                            $categoryCheck = $con->query("SELECT category, company_name, price, COUNT(category) FROM cart WHERE user_ID=".$userID." AND category LIKE '".$category[$i]."%' HAVING COUNT(category) == 1");
                            ...
                            if($categoryCheck !== false && $categoryCheck->num_rows > 0){
                                ...
                                $company = $categoryDisplay['company_name'];
                                $price = $categoryDisplay['price'];

                                echo "<div class=selection_items>";
